...
{activeTab === 'sessions' && (
  <div className="space-y-4">
    ...
    {/* ===================== AJOUT DU SYSTÈME DE TIMELINE / CALENDRIER DE JOURNÉE ===================== */}
    {currentSession && (
      <div className="bg-white p-6 rounded-xl border border-purple-200 shadow-sm space-y-6">
        <div className="border-b pb-3">
          <h3 className="text-sm font-black text-purple-950 uppercase tracking-wide">⏱️ Structurer le Déroulé de la Journée</h3>
          <p className="text-[11px] text-slate-400 mt-0.5">Ajoutez les blocs horaires de formation, pauses ou repas pour cette session.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-5 gap-6 items-start">
          
          {/* AFFICHAGE DE LA TIMELINE CHRONOLOGIQUE */}
          <div className="md:col-span-3 space-y-3">
            {(!currentSession.timeline_blocks || currentSession.timeline_blocks.length === 0) ? (
              <div className="border-2 border-dashed border-slate-100 rounded-xl p-8 text-center text-xs text-slate-400 italic">
                Aucun bloc de planning configuré pour le moment. Utilisez le mini-formulaire à droite.
              </div>
            ) : (
                  ...
                  <div key={block.id} className="relative bg-slate-50 border p-3 rounded-xl shadow-xs space-y-1 transition-all hover:bg-white group">
                    ...
                    <div className="flex justify-between items-start gap-2">
                      ...
                      <button 
                        ...
                        className="text-[10px] text-red-500 opacity-0 group-hover:opacity-100 font-bold cursor-pointer transition-all focus:outline-none"
                      >
                        Supprimer
                      </button>
                    </div>
          ...
          {/* FORMULAIRE D'INSERTION DE BLOC (BOÎTE DE DROITE) */}
          <div className="md:col-span-2 bg-slate-50 p-4 rounded-xl border border-slate-200/60 space-y-3">
            ...
            <form onSubmit={handleAddTimelineBlock} className="space-y-3 text-xs font-bold">
              <div>
                <label className="block text-slate-600 mb-0.5">Titre du bloc :</label>
                <input 
                  type="text" 
                  required 
                  placeholder="Ex: Palier 1 : Fondations / Repas libre" 
                  value={newBlockTitle} 
                  onChange={(e) => setNewBlockTitle(e.target.value)} 
                  className="w-full bg-white border rounded-lg p-2 font-medium focus:outline-none focus:border-purple-500" 
                />
              </div>
              <div>
                <label className="block text-slate-600 mb-0.5">Détail indicatif (Optionnel) :</label>
                <textarea 
                  rows="2" 
                  placeholder="Ex: Briefing de démarrage ou Pause café" 
                  value={newBlockDesc} 
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-slate-600 mb-0.5">Début :</label>
                  <input 
                    type="time" 
                    required 
                    value={newBlockStart} 
                    onChange={(e) => setNewBlockStart(e.target.value)} 
                    className="w-full bg-white border rounded-lg p-2 font-mono focus:outline-none focus:border-purple-500" 
                  />
                </div>
                <div>
                  <label className="block text-slate-600 mb-0.5">Fin :</label>
                  <input 
                    type="time" 
                    required 
                    value={newBlockEnd} 
              ...
              <button 
                type="submit" 
                className="w-full bg-purple-700 hover:bg-purple-800 text-white font-black py-2 rounded-lg text-xs mt-1 transition-all cursor-pointer shadow-xs"
              >
