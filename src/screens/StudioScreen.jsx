...
export default function StudioScreen({ trees = {}, setTrees, quests = [], setQuests }) {
  ...
  const handleTimelineDrop = (targetIndex) => {
    ...
    if (draggedPlanningItem.source === 'palette-palier') {
      ...
      // Construit le nom de manière dynamique sans fallback "Sans nom" si la donnée n'existe pas en BDD
      const blockName = floor.name && floor.name.trim() !== ""
        ? `🎯 Palier ${floor.floorId} : ${floor.name}`
                            ...
                            >
                              <div className="min-w-0">
                                <span className="font-extrabold text-xs text-purple-900 block truncate">
                                  {floor.name && floor.name.trim() !== "" ? `🎯 Palier ${floor.floorId} : ${floor.name}` : `🎯 Palier ${floor.floorId}`}
                                </span>
