              ...
              {mySessionsData.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {mySessionsData.map(sessionItem => {
                    ...
                    return (
                      <div key={sessionItem.session_code} className="bg-white border-2 border-slate-100 p-5 rounded-2xl shadow-sm hover:shadow-md transition-all flex flex-col justify-between gap-6 group relative overflow-hidden">
                        <div>
      ...
      {activeTab === 'parcours' && selectedSessionCode && selectedTreeId && trees[selectedTreeId] && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-4">
            ...
            {/* BARRE DE FILTRE INTELLIGENTE DU PARCOURS */}
            <div className="flex flex-wrap justify-between items-center gap-3 bg-white px-4 py-3 border rounded-xl shadow-sm">
              ...
              <div className="flex items-center gap-2">
                <select value={filterTheme} onChange={(e) => setFilterTheme(e.target.value)} className="bg-slate-50 border rounded-lg px-2 py-1 text-[11px] font-bold text-slate-700 cursor-pointer">
                  <option value="all">🌱 Toutes catégories</option>
                  <option value="env">🌍 RSE</option>
                  <option value="tech">⚙️ Tech</option>
                </select>

                <select value={filterMode} onChange={(e) => setFilterMode(e.target.value)} className="bg-slate-50 border rounded-lg px-2 py-1 text-[11px] font-bold text-slate-700 cursor-pointer">
                  <option value="all">🤝 Tous les modes</option>
                  <option value="solo">👤 Missions Solo</option>
                  <option value="collab">👥 Missions Co-op (🤝)</option>
                </select>
              </div>
            </div>
            ...
            {(() => {
              ...
              const filteredQuestsOnFloor = (quests.filter(q => (activeFloor.quests || []).includes(q.id))).filter(quest => {
                ...
                const matchMode = filterMode === 'all' || 
                                  (filterMode === 'collab' && isQuestCollab) || 
                                  (filterMode === 'solo' && !isQuestCollab);
              ...
              return (
                <div className="flex gap-4 items-stretch">
                  ...
                  <div className={`flex-1 bg-white border rounded-2xl p-5 shadow-sm space-y-4 relative ${triggerDropAnimation ? 'animate-drop-bounce' : ''}`}>
                    <div className="text-[11px] border-b pb-2 text-slate-400 font-bold uppercase">
                      Palier {activeFloor.floorId} — {trees[selectedTreeId].name}
                    </div>

                    {filteredQuestsOnFloor.length === 0 ? (
                      <p className="text-xs text-slate-400 italic text-center py-4">Aucune quête ne correspond à vos filtres sur ce palier.</p>
                    ) : (
